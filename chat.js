 ...
 */
const MODELS = [
  // اگر OPENROUTER_MODEL در Cloudflare تنظیم شده باشد، همان مدل اولویت دارد.
  // در غیر این صورت فقط مدل‌های رایگان به عنوان آخرین fallback استفاده می‌شوند.
  "openrouter/free",
...
/* =====================================================
   ...
   These are optional. Add the corresponding secret in
...
async function callGemini(env, messages) {
  ...
  try {
    ...
    return { answer, model: GEMINI_MODEL, provider: "Google Gemini" };
  } catch (e) {
    console.error("[YAR] Gemini error:", e);
    return null;
  }
}
...
async function callGroq(env, messages) {
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: GROQ_MODEL, messages, temperature: 0.7, max_tokens: 1000 })
    });
    const data = await response.json().catch(() => ({}));
    const answer = data?.choices?.[0]?.message?.content?.trim();
    if (!response.ok || !answer) return null;
    return { answer, model: GROQ_MODEL, provider: "Groq" };
  } catch (e) {
    console.error("[YAR] Groq error:", e);
    return null;
  }
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const hasGemini = !!String(env?.GEMINI_API_KEY || "").trim();
    const hasGroq = !!String(env?.GROQ_API_KEY || "").trim();
    const hasCloudflareAI = !!env?.AI && typeof env.AI.run === "function";

    // فقط وقتی هیچ Providerای تنظیم نشده باشد، خطای تنظیمات بده.
    if (!apiKey && !hasGemini && !hasGroq && !hasCloudflareAI) {
      return json({
        success: false,
        error: "❌ هیچ سرویس هوش مصنوعی در Backend تنظیم نشده است. GEMINI_API_KEY یا GROQ_API_KEY یا OPENROUTER_API_KEY را در Cloudflare Secrets اضافه کنید.",
    ...
    /* =================================================
       8. GEMINI / GROQ FALLBACKS
       These providers avoid OpenRouter's free-model daily quota.
    ================================================= */

    const gemini = await callGemini(env, messages);
    if (gemini?.answer) {
      return json({ success: true, reply: gemini.answer, message: gemini.answer, model: gemini.model, provider: gemini.provider, diagnostics: [{ model: gemini.model, status: 200, ok: true, provider: gemini.provider }] });
    }

    const groq = await callGroq(env, messages);
    if (groq?.answer) {
      return json({ success: true, reply: groq.answer, message: groq.answer, model: groq.model, provider: groq.provider, diagnostics: [{ model: groq.model, status: 200, ok: true, provider: groq.provider }] });
    }

    /* =================================================
       9. CALL OPENROUTER AS LAST FALLBACK
    ================================================= */
