...
async function verifyAdminSessionDetailed(req,env){
  const secret=adminSessionSecret(env);
  const owner=adminEmail(env);
  const token=adminSessionHeader(req);

  if(!secret)return {ok:false,code:"SESSION_SECRET_MISSING"};
  if(!owner)return {ok:false,code:"OWNER_EMAIL_MISSING"};
  if(!token)return {ok:false,code:"SESSION_MISSING"};

  const dot=token.lastIndexOf(".");
  if(dot<=0)return {ok:false,code:"SESSION_FORMAT_INVALID"};

  const encoded=token.slice(0,dot);
  const sig=token.slice(dot+1);

  let sigOk=false;
  try{sigOk=await hmacVerify(secret,encoded,sig)}catch(e){
    return {ok:false,code:"SESSION_HMAC_EXCEPTION",details:e?.message||String(e)}
  }
  if(!sigOk)return {ok:false,code:"SESSION_SIGNATURE_INVALID"};

  try{
    const raw=fromBase64url(encoded);
    const p=JSON.parse(new TextDecoder().decode(raw));

    if(p?.sub!=="yar-admin")return {ok:false,code:"SESSION_SUB_INVALID"};
    if(p?.email!==owner)return {ok:false,code:"SESSION_EMAIL_MISMATCH"};
    if(!(Number(p?.exp)>Date.now()))return {ok:false,code:"SESSION_EXPIRED"};

    return {ok:true,payload:{
      sub:p.sub,
      email_present:!!p.email,
      exp_present:!!p.exp,
      expired:false
    }};
  }catch(e){
    return {ok:false,code:"SESSION_PAYLOAD_INVALID",details:e?.message||String(e)}
  }
}

async function verifyAdminSession(req,env){
  return (await verifyAdminSessionDetailed(req,env)).ok;
}

function adminDiagnostic(request,env,code,extra={}){
  return json({
    success:false,
    authenticated:false,
    diagnostic:true,
    code,
    details:{
      env_email_present:!!adminEmail(env),
      env_password_present:!!adminPassword(env),
      env_secret_present:!!adminSessionSecret(env),
      header_session_present:!!adminSessionHeader(request),
...
async function adminAuthApi(request,env){
  if(request.method==="GET"){
    const result=await verifyAdminSessionDetailed(request,env);
    if(result.ok)return json({success:true,authenticated:true,valid:true,isAdmin:true,diagnostic:true});
    return adminDiagnostic(request,env,result.code,{
      token_present:!!adminSessionHeader(request),
      ...(result.details?{verification_details:result.details}:{})
    });
  }
  ...
  if(!owner||!password||!secret){
    return adminDiagnostic(request,env,"ADMIN_SECURITY_NOT_CONFIGURED",{
      missing:[
        !owner?"YAR_OWNER_EMAIL":null,
        !password?"YAR_ADMIN_PASSWORD":null,
  ...
  const suppliedPassword=text(b?.password);

  if(email!==owner||suppliedPassword!==password){
    return json({
      success:false,
  ...
  try{
    ...
    if(!session)return adminDiagnostic(request,env,"ADMIN_SESSION_CREATE_FAILED");

    // Immediately self-verify the exact token before returning it.
    const checkReq=new Request(new URL("/api/admin/session",request.url),{
      method:"GET",
      headers:{"X-Yar-Admin-Session":session}
    });
    const check=await verifyAdminSessionDetailed(checkReq,env);

    if(!check.ok){
      return adminDiagnostic(request,env,"LOGIN_CREATED_BUT_SELF_VERIFY_FAILED",{
        verification_code:check.code
      });
    }
    ...
    return json({
      ...
      session,
      expiresIn:28800,
      diagnostic_login:{credentials_ok:true,token_self_verified:true}
    });
  }catch(e){
    return adminDiagnostic(request,env,"LOGIN_EXCEPTION",{
