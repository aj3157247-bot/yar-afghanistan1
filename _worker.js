...
/* ================= DIRECT ADS / D1 ================= */
async function adsInit(db){await db.prepare(`CREATE TABLE IF NOT EXISTS direct_ads (id TEXT PRIMARY KEY,name TEXT NOT NULL,image TEXT NOT NULL DEFAULT '',video TEXT NOT NULL DEFAULT '',text TEXT NOT NULL,link TEXT NOT NULL DEFAULT '',start_date TEXT NOT NULL DEFAULT '',end_date TEXT NOT NULL DEFAULT '',duration INTEGER NOT NULL DEFAULT 15,status TEXT NOT NULL DEFAULT 'inactive',created_at INTEGER NOT NULL)`).run();await db.prepare(`CREATE TABLE IF NOT EXISTS direct_ad_events (id INTEGER PRIMARY KEY AUTOINCREMENT,ad_id TEXT NOT NULL,event_type TEXT NOT NULL,created_at INTEGER NOT NULL)`).run();await db.prepare(`CREATE INDEX IF NOT EXISTS idx_direct_ad_events_ad_type ON direct_ad_events(ad_id,event_type)`).run()}
function normalizeAdRow(r){return {id:r.id,name:r.name,image:r.image||"",video:r.video||"",text:r.text||"",link:r.link||"",start:r.start_date||"",end:r.end_date||"",duration:Math.max(5,Math.min(60,Number(r.duration)||15)),status:r.status||"inactive",createdAt:Number(r.created_at)||0}}
async function adsApi(request,env){const db=env?.DB;if(!db)return json({success:false,error:"D1 binding DB is not configured.",code:"NO_DB"},503);try{await adsInit(db)}catch(e){return json({success:false,error:"Failed to initialize ads database.",code:"ADS_DB_INIT_FAILED",details:e?.message||String(e)},503)}const u=new URL(request.url),path=u.pathname;
 if(request.method==="GET"&&path==="/api/ads"){const today=new Date().toISOString().slice(0,10);const {results=[]}=await db.prepare(`SELECT id,name,image,video,text,link,start_date,end_date,duration,status,created_at FROM direct_ads WHERE status='active' AND (start_date='' OR start_date<=?) AND (end_date='' OR end_date>=?) ORDER BY created_at DESC`).bind(today,today).all();return json({success:true,ads:results.map(normalizeAdRow)},{status:200})}
 if(request.method==="POST"&&path==="/api/ads/event"){let b={};try{b=await request.json()}catch{return json({success:false,error:"Invalid JSON.",code:"INVALID_JSON"},400)}const type=b?.type==="click"?"click":b?.type==="impression"?"impression":"",id=text(b?.adId).trim().slice(0,160);if(!type||!id)return json({success:false,error:"Invalid ad event.",code:"INVALID_AD_EVENT"},400);const exists=await db.prepare("SELECT id FROM direct_ads WHERE id=? LIMIT 1").bind(id).first();if(!exists)return json({success:false,error:"Ad not found.",code:"AD_NOT_FOUND"},404);await db.prepare("INSERT INTO direct_ad_events(ad_id,event_type,created_at) VALUES(?,?,?)").bind(id,type,Date.now()).run();return json({success:true})}
 if(!(await verifyAdminSession(request,env)))return json({success:false,error:"Owner access required.",code:"OWNER_ONLY"},403);
 if(request.method==="GET"&&path==="/api/ads/admin"){const {results=[]}=await db.prepare(`SELECT a.*,COALESCE((SELECT COUNT(*) FROM direct_ad_events e WHERE e.ad_id=a.id AND e.event_type='impression'),0) impressions,COALESCE((SELECT COUNT(*) FROM direct_ad_events e WHERE e.ad_id=a.id AND e.event_type='click'),0) clicks FROM direct_ads a ORDER BY a.created_at DESC`).all();return json({success:true,ads:results.map(r=>({...normalizeAdRow(r),impressions:Number(r.impressions)||0,clicks:Number(r.clicks)||0}))})}
 if(request.method==="POST"&&path==="/api/ads"){let b={};try{b=await request.json()}catch{return json({success:false,error:"Invalid JSON.",code:"INVALID_JSON"},400)}const id=text(b?.id).trim().slice(0,160)||`ad_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,name=text(b?.name).trim().slice(0,200),adText=text(b?.text).trim().slice(0,2000);if(!name||!adText)return json({success:false,error:"نام و متن تبلیغ الزامی است.",code:"AD_FIELDS_REQUIRED"},400);const duration=Math.max(5,Math.min(60,Number(b?.duration)||15)),status=b?.status==="active"?"active":"inactive",start=text(b?.start).trim().slice(0,20),end=text(b?.end).trim().slice(0,20);if(start&&end&&start>end)return json({success:false,error:"تاریخ شروع نباید بعد از تاریخ پایان باشد.",code:"INVALID_AD_DATES"},400);const createdAt=Number(b?.createdAt)||Date.now();await db.prepare(`INSERT INTO direct_ads(id,name,image,video,text,link,start_date,end_date,duration,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,image=excluded.image,video=excluded.video,text=excluded.text,link=excluded.link,start_date=excluded.start_date,end_date=excluded.end_date,duration=excluded.duration,status=excluded.status`).bind(id,name,text(b?.image).trim().slice(0,4000),text(b?.video).trim().slice(0,4000),adText,text(b?.link).trim().slice(0,4000),start,end,duration,status,createdAt).run();return json({success:true,id})}
 const match=path.match(/^\/api\/ads\/([^/]+)$/);if(match&&request.method==="PATCH"){const id=decodeURIComponent(match[1]),b=await request.json().catch(()=>({})),status=b?.status==="active"?"active":"inactive",r=await db.prepare("UPDATE direct_ads SET status=? WHERE id=?").bind(status,id).run();if(!r?.meta?.changes)return json({success:false,error:"Ad not found.",code:"AD_NOT_FOUND"},404);return json({success:true,id,status})}if(match&&request.method==="DELETE"){const id=decodeURIComponent(match[1]),r=await db.prepare("DELETE FROM direct_ads WHERE id=?").bind(id).run();await db.prepare("DELETE FROM direct_ad_events WHERE ad_id=?").bind(id).run();if(!r?.meta?.changes)return json({success:false,error:"Ad not found.",code:"AD_NOT_FOUND"},404);return json({success:true,id})}return json({success:false,error:"Ads route not found.",code:"NOT_FOUND"},404)}
