...
const GEMINI_FILE_MODELS = ["gemini-3.6-flash", "gemini-3.7-flash", "gemini-2.5-flash"];
...
async function fileBinaryApi(request, env) {
  ...
  try {
    ...
    if(ext==='pdf' && key(env,'GEMINI_API_KEY')) {
      ...
      const prompt=`You are Yar Afghanistan's document analysis assistant. ${language==='ps'?'Answer in Afghan Pashto.':language==='en'?'Answer in English.':'Answer in natural Afghan Dari.'} Read the attached PDF itself and answer the user's request from its contents. Do not claim you cannot open attachments. Do not invent facts. If the PDF is scanned, use OCR/vision if available. File: ${name}
User request: ${question}`;
      const modelErrors=[];
      for(const model of GEMINI_FILE_MODELS){
        try{
          const gr=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(key(env,'GEMINI_API_KEY'))}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{role:'user',parts:[{text:prompt},{inlineData:{mimeType:'application/pdf',data}}]}],generationConfig:{temperature:.2,maxOutputTokens:3500}})});
          const gd=await gr.json().catch(()=>({}));
          if(gr.ok){const answer=(gd?.candidates?.[0]?.content?.parts||[]).map(p=>p?.text||'').join('\n').trim();if(answer)return json({success:true,reply:answer,message:answer,provider:'Gemini',model:gd?.model||model,file:name,nativePdf:true});}
          modelErrors.push(`${model}: ${gd?.error?.message||`HTTP ${gr.status}`}`);
        }catch(e){modelErrors.push(`${model}: ${e?.message||String(e)}`)}
      }
      // Keep the actual provider error so the UI no longer falls back to a misleading generic AI answer.
      if(modelErrors.length) console.error('[YAR] PDF Gemini models failed:',modelErrors.join(' | '));
    }
    let extracted='';
