...
async function adsApi(request,env){
  ...
  if(request.method==='GET'&&path==='/api/ads'){
    ...
    const {results=[]}=await db.prepare(`SELECT a.*,COALESCE((SELECT COUNT(*) FROM direct_ad_events e WHERE e.ad_id=a.id AND e.event_type='impression'),0) impressions,COALESCE((SELECT COUNT(*) FROM direct_ad_events e WHERE e.ad_id=a.id AND e.event_type='click'),0) clicks FROM direct_ads a WHERE a.status='active' AND (a.start_date='' OR a.start_date<=?) AND (a.end_date='' OR a.end_date>=?) ORDER BY a.created_at DESC`).bind(today,today).all();
    return json({success:true,ads:results.filter(r=>(!Number(r.max_impressions)||Number(r.impressions)<Number(r.max_impressions))&&(!Number(r.max_clicks)||Number(r.clicks)<Number(r.max_clicks))).map(normalizeAdRow)});
  }
  if(request.method==='POST'&&path==='/api/ads/event'){
    ...
    const ad=await db.prepare('SELECT id,max_impressions,max_clicks FROM direct_ads WHERE id=? LIMIT 1').bind(id).first();if(!ad)return json({success:false,error:'Ad not found.',code:'AD_NOT_FOUND'},404);
