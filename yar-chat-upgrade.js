/* Yar AI Chat Experience v2 - ChatGPT-style conversations + unified attachments */
(function(){
  'use strict';
  ...
  async function analyzeZipFile(file,question,fix){const {zip,files}=await zipText(file);const r=await fetch('/api/project',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({instruction:(question||'پروژه را کامل تحلیل کن.')+(fix?'\nدر صورت نیاز فایل‌های اصلاح‌شده را نیز برگردان.':'\nفقط تحلیل و پیشنهاد بده و هیچ فایل را تغییر نده.'),files})});const d=await r.json().catch(()=>({}));if(!r.ok||!d.success)throw new Error(d.error||'تحلیل پروژه ناموفق بود.');let result=d.summary||'تحلیل پروژه انجام شد.';if(fix&&Array.isArray(d.files)&&d.files.length){for(const f of d.files){if(f.action==='delete')zip.remove(f.path);else zip.file(f.path,f.content)}const blob=await zip.generateAsync({type:'blob'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name.replace(/\.zip$/i,'')+'-fixed.zip';a.textContent='📦 دریافت ZIP اصلاح‌شده';a.className='yar-project-download';a.style.cssText='display:inline-block;margin:10px 0;padding:10px 14px;border-radius:12px;background:#d8ad3f;color:#151007;font-weight:800;text-decoration:none';messages.appendChild(a)}return result}

  function menuFile(kind){hidden.accept=kind==='image'?'image/*':kind==='video'?'video/*':kind==='zip'?'.zip,application/zip':'.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.md,.json,.html,.css,.js,.ts,.jsx,.tsx,.py,.java,.php,.sql,.xml,.yaml,.yml,.rtf';hidden.multiple=kind!=='zip';hidden.dataset.kind=kind;hidden.click();$('yarPlusMenu').classList.remove('show')}
